...
export function getBoard() {
  return async () => {
    ...
    try {
      const response = await request.get('/api/kanban/board');
...
export function createColumn(newColumn: { name: string }) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.post('/api/kanban/columns/new', newColumn);
...
export function updateColumn(columnId: string, updateColumn: KanbanColumn) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.post('/api/kanban/columns/update', {
        columnId,
        updateColumn,
      });
...
export function deleteColumn(columnId: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      await request.post('/api/kanban/columns/delete', { columnId });
