...
export function createEvent(newEvent: Omit<EventInput, 'id'>) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.post('/api/calendar/events/new', newEvent);
...
export function updateEvent(
  ...
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.post('/api/calendar/events/update', {
        eventId,
        updateEvent,
      });
...
export function deleteEvent(eventId: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      await request.post('/api/calendar/events/delete', { eventId });
