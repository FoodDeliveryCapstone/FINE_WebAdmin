...
export function getConversation(conversationKey: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.get('/api/chat/conversation', {
...
export function markConversationAsRead(conversationId: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      await request.get('/api/chat/conversation/mark-as-seen', {
...
export function getParticipants(conversationKey: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await request.get('/api/chat/participants', {
        params: { conversationKey },
      });
